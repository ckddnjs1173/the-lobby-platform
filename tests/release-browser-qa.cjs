...
async function assertNoHorizontalOverflow(page, label) {
  const metrics = await page.evaluate(() => ({ viewport: window.innerWidth, scrollWidth: document.documentElement.scrollWidth, bodyWidth: document.body.scrollWidth }));
  assert(metrics.scrollWidth <= metrics.viewport + 2 && metrics.bodyWidth <= metrics.viewport + 2, `${label}:HORIZONTAL_OVERFLOW:${JSON.stringify(metrics)}`);
}
...
async function capture(page, viewportName, pageName) {
  await page.screenshot({ path: path.join(ARTIFACT_DIR, `${viewportName.toLowerCase()}-${pageName}.png`), fullPage: true });
}
...
async function runViewport(browser, name, viewport) {
  ...
  page.on("console", (message) => {
    if (message.type() === "error") {
      ...
      if (!/Firebase|identitytoolkit|ERR_NAME_NOT_RESOLVED|fetch/i.test(text)) pageErrors.push(`console:${text}`);
    }
  });
  ...
  for (const [route, screenshotName] of [["/login", "login"], ["/register", "register"], ["/b2b-admin/login", "b2b-login"]]) {
  ...
  if (pageErrors.length > 0) throw new Error(`${name}:BROWSER_ERRORS:${pageErrors.join(" | ")}`);
