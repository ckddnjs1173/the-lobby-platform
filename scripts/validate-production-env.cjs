...
if (read("NEXT_PUBLIC_FIREBASE_PROJECT_ID") !== read("FIREBASE_ADMIN_PROJECT_ID")) {
...
if (publicLaunchMode) {
  ...
  if (missingPublicLaunch.length > 0) {
    ...
    for (const key of missingPublicLaunch) console.error(`- ${key}`);
    process.exit(1);
  }

  const placeholderPublicLaunch = publicLaunchRequiredKeys.filter((key) => looksLikePlaceholder(read(key)));
  if (placeholderPublicLaunch.length > 0) {
    console.error("PRODUCTION_ENV_VALIDATION_FAILED");
    console.error("Public launch values must not contain placeholder text:");
    for (const key of placeholderPublicLaunch) console.error(`- ${key}`);
    process.exit(1);
  }
  ...
  if (siteUrl.protocol !== "https:") fail("NEXT_PUBLIC_SITE_URL must use HTTPS for public launch.");
  if (!read("NEXT_PUBLIC_PRIVACY_CONTACT_EMAIL").includes("@")) {
    fail("NEXT_PUBLIC_PRIVACY_CONTACT_EMAIL must contain an email address.");
  }
  if (!read("NEXT_PUBLIC_CUSTOMER_SUPPORT_CONTACT").includes("@")) {
    fail("NEXT_PUBLIC_CUSTOMER_SUPPORT_CONTACT must contain an email address for the current no-phone public contact policy.");
  }
...
if (emailAutomationRequested) {
  if (communicationProvider !== "RESEND") fail("When email automation is configured, COMMUNICATION_EMAIL_PROVIDER must be RESEND.");
  if (!resendApiKey || !communicationFromEmail) fail("Email automation requires both RESEND_API_KEY and COMMUNICATION_FROM_EMAIL.");
  if (!communicationFromEmail.includes("@")) fail("COMMUNICATION_FROM_EMAIL must contain an email address.");
